 ...
 *
 */
public class PaginatedResultPanel extends JPanel implements Serializable,
	...
	private static final Logger logger = Logger.getLogger(PaginatedResultPanel.class);
	
	public static final int MIN_RECORDS_PER_PAGE = 500;
    ...
    public PaginatedResultPanel(JFrame parentFrame, ConnectionProperties connectionProperties, String queryString) {
    ...
    private int getTotalRecords(String filterSubQuery){
    	...
    	if(null != queryExecutionService){
    		try {
				totalRows = queryExecutionService.getTotalRecords(connectionProperties, queryString, filterSubQuery);
    ...
    private int getTotalRecords(){
    	...
    	if(null != queryExecutionService){
    		try {
				totalRows = queryExecutionService.getTotalRecords(connectionProperties, queryString);
			} catch (DbexException e) {
				DisplayUtils.displayMessage(getParentFrame(), e.getMessage(), DisplayTypeEnum.ERROR);
			}
    	}
    ...
    public void showTableData() {
    	...
    	try {
			targetTable.setModel(dataTableTableModelFactory.getResultSetTableModel(
					connectionProperties, queryString, rowNumFrom, rowNumTo));
    ...
    public void showTableData(String filterSubQuery) {
		...
		paginationResult.setEndRow(paginationResult.getStartRow() + paginationResult.getRowsPerPage());
    	try {
    		DataTableTableModel tableModel = dataTableTableModelFactory.getResultSetTableModel(connectionProperties, queryString, filterSubQuery);
